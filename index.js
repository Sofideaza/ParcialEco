...
app.post('/users/register', (req, res) => {
  try {
    ...
    users.push(newUser);
    db.save('users', users);
    res.status(201).json({ id: newUser.id, name: newUser.name, balance: newUser.balance });
  } catch {
...
app.post('/items/:id/bid', (req, res) => {
  try {
    ...
    const idx = user.bids.findIndex(bid => bid.itemId === itemId);
    if (idx !== -1) {
      user.bids[idx].amount = amount;
    } else {
    ...
    res.status(200).json({ itemId: item.id, highestBid: item.highestBid, highestBidder: item.highestBidder });
  } catch {
    res.status(500).json({ error: 'error del servidor' });
  }
});
...
app.post('/auction/openAll', (req, res) => {
  try {
    ...
    res.status(200).json({ auction: 'abierta', startTime: new Date().toISOString() });
  } catch {
  ...
  try {
    ...
    for (const item of items) {
      if (item.highestBidder) {
        item.sold = true;
        const winner = users.find(u => u.name === item.highestBidder);
        if (winner) {
          winner.balance -= item.highestBid;
          winner.bids = winner.bids.filter(b => b.itemId !== item.id);
        }
        results.push({
          itemId: item.id,
          item: item.name,
    ...
    db.save('items', items);
    db.save('users', users);
    res.status(200).json({ auction: 'cerrada', results });
  } catch {
...
app.listen(PORT, () => {
  ...
  console.log(`Monitor: http://localhost:${PORT}/monitor-app`);
  console.log(`Jugadores: http://localhost:${PORT}/players-app`);
});
